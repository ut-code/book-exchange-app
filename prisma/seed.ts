...
const main = async () => {
  ...
  const algo_book = await prisma.book.create({
    data: {
      ...
      thumbnail:
        "https://m.media-amazon.com/images/I/51oWwpzibRL._SX218_BO1,204,203,200_QL40_ML2_.jpg",
  ...
  const golang_book = await prisma.book.create({
    data: {
      ...
      language: "ja",
      description:
        "Go言語でつくるインタプリタは、Go言語を使ってインタプリタを作ることで、プログラミング言語の仕組みを学ぶことができる本です。本書では、Go言語を使って、インタプリタを作りながら、プログラミング言語の仕組みを学ぶことができます。",
      thumbnail:
        "https://m.media-amazon.com/images/I/51sLCPa8DBL._SR600%2C315_PIWhiteStrip%2CBottomLeft%2C0%2C35_PIStarRatingFOURANDHALF%2CBottomLeft%2C360%2C-6_SR600%2C315_ZA18%2C445%2C290%2C400%2C400%2CAmazonEmberBold%2C12%2C4%2C0%2C0%2C5_SCLZZZZZZZ_FMpng_BG255%2C255%2C255.jpg",
  ...
  const deeplearning_book = await prisma.book.create({
    data: {
      ...
      language: "ja",
      description:
        "本書は、ディープラーニングの基礎をゼロから学ぶことができる、オライリー・ジャパンの人気シリーズ「ゼロから作る」の第4弾です。本書では、ディープラーニングの基礎をゼロから学ぶことができます。本書では、ディープラーニングの基礎をゼロから学ぶことができます。",
      thumbnail:
        "https://www.oreilly.co.jp/books/images/picture_large978-4-87311-975-5.jpeg",
  ...
  const exchangeRequest2 = await prisma.exchangeRequest.create({
    data: {
      ...
      requester_books: {
        connect: [{ id: algo_book.id }],
      },
      addressee_books: {
        connect: [{ id: golang_book.id }, { id: deeplearning_book.id }],
      },
      status: "requested",
    },
  });
